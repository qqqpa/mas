...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `برمجه وكتابه بودي ...`")
    update_avail = updater()
    if update_avail:
        await msg.edit("👤 يمكن شراء بوت بحقوقك كامل \n\n• او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : @a_bd80 .")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit("▹￤ يمكن شراء بوت بحقوقك كامل \n\n▹￤ او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : @a_bd80 .", disable_web_page_preview=True)
